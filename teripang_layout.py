...
with tab1:
    ...
    Hewan ini menjadi komoditi ekspor non-migas yang penting bagi Indonesia sejak 200 tahun yang lalu dan menghasilkan devisa negara dari berbagai negara tujuan ekspor.
    """
...
with tab2:
    ...
    with st.expander("Penjelasan Produksi Teripang di Indonesia (2019-2022):"):
    ...
    Salah satu provinsi yang memiliki potensi besar untuk melakukan ekspor teripang adalah Nusa Tenggara Barat (NTB).
    Walaupun NTB memiliki banyak keanekaragaman jenis teripang, NTB tidak memproduksi teripang dan mengekspor teripang ke luar Indonesia.
...
with tab3:
    ...
    with st.expander("Penjelasan nilai ekspor dan volume ekspor teripang di Indonesia (2014-2022):"):
    ...
    plt.xlabel('Jumlah Nilai Impor Teripang (ratus juta USD)')
    plt.ylabel('Negara Tujuan Ekspor (Pengimpor) Teripang')
    plt.title('Negara-Negara Tujuan Ekspor (Pengimpor) Teripang di Dunia')

    # Menampilkan grafik menggunakan Streamlit
    st.pyplot(fig4)


    # Paragraf Penjelasan Negara-Negara Tujuan Ekspor Teripang yang ingin ditampilkan
    paragraf5 = """
    ...
    # Menampilkan paragraf5 di Streamlit
    with st.expander("Penjelasan negara-negara tujuan ekspor teripang:"):
        st.markdown(f'<p style="font-family: Arial; font-size: 16px;">{paragraf5}', unsafe_allow_html=True)
    


    # Menghilangkan data "world" dari DataFrame
    df6_filtered = df6[df6['exporters'] != 'World']

    # Mengelompokkan data berdasarkan negara pengekspor dan menghitung total nilai ekspor
    total_export_value = df6_filtered.groupby('exporters')['fob_value'].sum().sort_values(ascending=False)

    # Mengambil 10 negara pengekspor terbanyak
    top_10_exporters_countries = total_export_value.head(10)

    # Membuat grafik horizontal bar chart
    fig6, ax = plt.subplots(figsize=(10, 8))
    top_10_exporters_countries.plot(kind='barh', color='skyblue')
    plt.xlabel('Jumlah Nilai Ekspor Teripang (ratus juta USD)')
    plt.ylabel('Negara Pengekspor Teripang di Dunia')
    plt.title('Negara-Negara Pengekspor Teripang di Dunia')
    ...
    # Paragraf Penjelasan Negara-Negara Pengekspor Teripang yang ingin ditampilkan
    paragraf6 = """
    Selain Indonesia, lima negara pengekspor teripang terbanyak di dunia adalah Jepang, China, Korea, dan Meksiko.
    Total nilai ekspor teripang di Indonesia pada tahun 2018 hingga 2022 adalah sebesar USD 25.057.594, sedangkan total nilai ekspor teripang di dunia pada tahun 2018 hingga 2022 adalah sebesar USD 428.609.000 sehingga didapatkan pangsa pasar relatif Indonesia dalam ekspor teripang sepanjang tahun 2018 hingga 2022 adalah sebesar 5,8%.
    Meskipun pangsa pasar relatif sebesar 5,8% masih tergolong kecil dibandingkan total nilai ekspor teripang di dunia, kontribusi Indonesia dalam ekspor teripang di dunia masih tetap signifikan.
    Oleh karena itu, peningkatan mutu kualitas ekspor teripang di Indonesia perlu dilakukan agar ekspor teripang di Indonesia dapat meningkat.
    ...
    # Menampilkan paragraf5 di Streamlit
    with st.expander("Penjelasan negara-negara pengekspor teripang:"):
        st.markdown(f'<p style="font-family: Arial; font-size: 16px;">{paragraf6}', unsafe_allow_html=True)
...
with tab4:
    ...
    # Menampilkan paragraf7 di Streamlit
    st.markdown(f'<p style="font-family: Arial; font-size: 16px;">{paragraf7}</p>', unsafe_allow_html=True)
